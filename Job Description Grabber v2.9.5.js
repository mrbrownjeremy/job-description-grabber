...
// @match        *://*.nyfa.org/*
// @match        *://*.pacareerlink.pa.gov/*
// @match        *://*.gusto.com/*
// @match        *://*.comeet.com/*
// @include      /^https?:\/\/careers\..+/
// @include      /^https?:\/\/jobs\..+/
// @include      /^https?:\/\/job\..+/
// @include      /^https?:\/\/apply\..+/
// @include      /^https?:\/\/hiring\..+/
// @include      /^https?:\/\/hire\..+/
// @include      /^https?:\/\/talent\..+/
// @include      /^https?:\/\/work\..+/
// @include      /^https?:\/\/.+\/jobs?\//
// @include      /^https?:\/\/.+\/careers?\//
// @include      /^https?:\/\/.+\/positions?\//
// @include      /^https?:\/\/.+\/openings?\//
// @include      /^https?:\/\/.+\/postings?\//
// @include      /^https?:\/\/.+\/requisitions?\//
// @grant        GM_setValue
// @grant        GM_getValue
...
(function () {
  ...
  function extractJobData() {
    ...
    const badEmployerValue = (s) => /^(company|employer|organization|business)$/i.test(clean(s));
      ...
      try {
        ...
        for (const j of jobs) {
          ...
          if (type === 'JobPosting' || (Array.isArray(type) && type.includes('JobPosting'))) {
            ...
            data.description = j.description ? stripHtml(j.description) : data.description;
          }
        }
      } catch (e) {}
    }
    ...
    if (isGusto) {
      ...
      if (h1) {
        ...
        if (employerSpan) data.employer = employerSpan.text;

        const titleSpan = spans.find(x => /text-3xl|text-4xl/i.test(x.cls));
        if (titleSpan) data.position = titleSpan.text;
        ...
        if (subtitleSpan) {
          ...
          if (parts[0]) data.location = parts[0];
          if (parts[1]) data.ftPtCT = mapEmploymentType(parts[1]) || data.ftPtCT;
        }
      }

      if (!data.employer) {
        const brandSpan = document.querySelector('h1 span[class*="uppercase"][class*="font-semibold"], h1 span[class*="uppercase"][class*="tracking-wide"]');
        if (brandSpan) data.employer = clean(brandSpan.textContent);
      }

      if (!data.position) {
        const gustoTitle = document.querySelector('h1 span[class*="text-3xl"], h1 span[class*="text-4xl"]');
        if (gustoTitle) data.position = clean(gustoTitle.textContent);
      }

      const descHeading = [...document.querySelectorAll('h1, h2, h3, h4')].find(el => clean(el.textContent).toLowerCase() === 'description');
      if (descHeading) {
        const richText = descHeading.parentElement?.querySelector('.rich-text-container');
        if (richText) {
          data.description = clean(richText.innerText || richText.textContent || '');
        }
      }

      if (!data.employer) {
        const crumb = [...document.querySelectorAll('nav[aria-label="Breadcrumb"] a')].find(a => /careers\s+at\s+/i.test(a.textContent));
        if (crumb) {
    ...
    if ((!data.location || !data.ftPtCT) && !isGusto) {
  ...
  function extractLabeledFields() {
    ...
    document.querySelectorAll('strong, b').forEach(el => {
      ...
      if (value.trim()) set(key, value, true);
    });
  ...
  function doShowSettings() {
    ...
    const modal = document.createElement('div');
    modal.id = 'jdg-modal';
    modal.style.cssText = 'position:fixed;top:60px;right:24px;pointer-events:all;max-width:480px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;';
