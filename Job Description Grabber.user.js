...
(function () {
  ...
  // Domains always blocked, regardless of allowlist — script never activates here
  const BLACKLIST = [
    'coda.io',
    'docs.google.com',
  ];
  ...
  function pageMatchesAllowlist() {
    ...
    if (BLACKLIST.some(d => host.includes(d.toLowerCase()))) return false;
  ...
  function extractDescriptionSections(text) {
    if (!text) return { responsibilities: '', requirements: '', benefits: '' };
    ...
    const RESP_RE = /^(responsibilities(\s+include)?|what (you('ll|'d| will) do|you('re)? ?(going to )?doing|you'd be doing)|your (role|responsibilities)|key responsibilities|day[\s-]to[\s-]day|job duties( (and|&) responsibilities)?|job description|duties( (and|&) responsibilities)?|about the role|in this role( you will)?|role overview|(primary|core|essential|main|key) (responsibilities|duties|functions)|position responsibilities|you will be responsible for|what does the job involve|the role|role (and|&) responsibilities)[\s:]*$/i;
    ...
    // Detects "Requirements Some content here..." — heading keyword fused with content on same line.
    ...
    const checkInlineHeading = (ln) => {
      ...
      for (const [re, key] of pairs) {
        const m = ln.match(re);
        if (m) return { key, content: ln.slice(m[0].length).trim() };
      }
      return null;
    };
    ...
    for (const line of lines) {
      ...
      if (inlineHd) {
        section = inlineHd.key;
        pendingBullet = false;
        if (inlineHd.content) pushItem(inlineHd.content);
        continue;
      }
      ...
      <div class="jdg-grid">
        ...
        <div class="jdg-field">
          <div style="display:flex;align-items:center;gap:6px;margin-bottom:2px;">
            <label style="margin:0;flex-shrink:0;">Description</label>
            <button id="jdg-desc-edit-btn" style="padding:1px 8px;font-size:11px;border:1px solid #ccc;border-radius:4px;background:#fff;cursor:pointer;color:#555;flex-shrink:0;">Edit</button>
          </div>
        ...
        <div class="jdg-field">
          <div style="display:flex;align-items:center;gap:6px;margin-bottom:2px;">
            <label style="margin:0;flex-shrink:0;">Benefits</label>
            <button id="jdg-ben-edit-btn" style="padding:1px 8px;font-size:11px;border:1px solid #ccc;border-radius:4px;background:#fff;cursor:pointer;color:#555;flex-shrink:0;">Edit</button>
          </div>
          <div class="jdg-desc-preview" id="jdg-ben-preview" style="max-height:1.6em;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">${escHtml(data.benefits.slice(0, 200))}</div>
