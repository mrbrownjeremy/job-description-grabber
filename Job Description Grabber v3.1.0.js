  ...
  function doShowSettings() {
    ...
    modal.innerHTML = `
      ...
      <div id="jdg-settings">
        ...
        <div class="jdg-settings-section">
          <div class="jdg-collapsible-header" id="jdg-sites-toggle">
            <h3>Site Matching</h3>
            <span class="jdg-collapsible-arrow" id="jdg-sites-arrow">▼</span>
          </div>
          <div class="jdg-collapsible-body collapsed" id="jdg-sites-body">
            <div class="jdg-subsection">
              <h4>Domains</h4>
              <ul class="jdg-industry-list" id="jdg-domain-list">
                ${domains.map((d, i) => matchItem(d, i, 'domain')).join('')}
              </ul>
              <div class="jdg-add-industry">
                <input type="text" id="jdg-domain-new" placeholder="e.g. bamboohr.com">
                <button class="jdg-btn-sm jdg-btn-add" id="jdg-domain-add">Add</button>
              </div>
            </div>
            <div class="jdg-subsection" style="margin-top:14px;">
              <h4>URL Patterns</h4>
              <ul class="jdg-industry-list" id="jdg-pattern-list">
                ${patterns.map((p, i) => matchItem(p, i, 'pattern')).join('')}
              </ul>
              <div class="jdg-add-industry">
                <input type="text" id="jdg-pattern-new" placeholder="e.g. /careers/ or jobs.">
                <button class="jdg-btn-sm jdg-btn-add" id="jdg-pattern-add">Add</button>
              </div>
            </div>
          </div>
        </div>
    ...
    // Collapsible toggles
    const makeToggle = (toggleId, bodyId, arrowId) => {
      modal.querySelector(`#${toggleId}`).addEventListener('click', () => {
    ...
    makeToggle('jdg-sites-toggle', 'jdg-sites-body', 'jdg-sites-arrow');
    makeToggle('jdg-ind-toggle',   'jdg-ind-body',   'jdg-ind-arrow');

    // Domain list
    const domainListEl = modal.querySelector('#jdg-domain-list');
    ...
    modal.querySelector('#jdg-domain-add').addEventListener('click', () => {
      const input = modal.querySelector('#jdg-domain-new');
    ...
    // Pattern list
    const patternListEl = modal.querySelector('#jdg-pattern-list');
    ...
    modal.querySelector('#jdg-pattern-add').addEventListener('click', () => {
      const input = modal.querySelector('#jdg-pattern-new');
      const val = input.value.trim();
    ...
    // Close — single instance guard means just removing the overlay is enough
